...
// Define the 5 launch steps with their subtasks
const LAUNCH_STEPS = [
  {
    number: 1,
...
export async function POST(req: Request) {
  try {
    ...
    // Create launch with steps stored as JSON
    const stepsWithIds = LAUNCH_STEPS.map(step => ({
      ...step,
      id: nanoid(),
      isCompleted: false,
      completedAt: null,
    ...
    try {
      await db.insert(businessLaunch).values({
        ...
        progress: 0,
        status: 'in_progress',
        completedSteps: JSON.stringify(stepsWithIds),
        createdAt: new Date(),
        updatedAt: new Date(),
      })
      console.log('[v0] Business launch created successfully:', launchId)
    } catch (dbError) {
